...
class BattleArenaController(arena_info.IArenaController):
    ...
    def __updateTeamData(self, isEnemy, arenaDP, isFragsUpdate = True):
        ...
        inviteSendingProhibited = isEnemy or self.prbInvites.getSentInviteCount() >= 100 or not g_settingsCore.getSetting('receiveInvitesInBattle')
        ...
        for index, (vInfoVO, vStatsVO, viStatsVO) in enumerate(arenaDP.getTeamIterator(isEnemy)):
            ...
            valuesHash = self.__makeHash(index, playerFullName, vInfoVO, vStatsVO, viStatsVO, ctx, userGetter, isSpeaking, isMenuEnabled, regionGetter, playerAccountID, inviteSendingProhibited)
    ...
    def __makeHash(self, index, playerFullName, vInfoVO, vStatsVO, viStatsVO, ctx, userGetter, isSpeaking, isMenuEnabled, regionGetter, playerAccountID, inviteSendingProhibited):
        ...
        return {'position': index + 1,
         ...
         'dynamicSquad': self.__getDynamicSquadData(dbID, playerAccountID, isInSquad=squadIndex > 0, inviteSendingProhibited=isInvitesForbidden)}

    def __getDynamicSquadData(self, userDBID, currentAccountDBID, isInSquad, inviteSendingProhibited):
        INVITE_DISABLED = 0
        IN_SQUAD = 1
        INVITE_AVAILABLE = 2
        INVITE_SENT = 3
        ...
        if inviteSendingProhibited:
            ...
            for invite in self.prbInvites.getInvites():
                if invite.type == PREBATTLE_TYPE.SQUAD:
                    ...
                    if invite.creatorDBID == userDBID and invite.receiverDBID == currentAccountDBID and inviteState == PRB_INVITE_STATE.PENDING:
                        state = INVITE_RECEIVED
                    elif invite.creatorDBID == currentAccountDBID and userDBID == invite.receiverDBID and inviteState == PRB_INVITE_STATE.PENDING:
                        state = INVITE_SENT

        if isInSquad and state != INVITE_RECEIVED:
            state = IN_SQUAD
        return state
