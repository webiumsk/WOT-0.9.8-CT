...
class UnitFunctional(_UnitFunctional):
    ...
    def validateLevels(self, stats = None, flags = None, vInfo = None, pInfo = None):
        ...
        return (self._actionValidator.canCreatorDoAction(pInfo, stats, flags, vInfo)[0], self._actionValidator.getRestrictionByLevel(stats, flags))
        ...
        if ctx.isRequestToStart():
            ...
            if stats.curTotalLevel > stats.maxTotalLevel:
                ...
                if callback:
                    callback(False)
                return
            self._requestsProcessor.doRequest(ctx, 'startBattle', vehInvID=ctx.selectVehInvID)
        else:
