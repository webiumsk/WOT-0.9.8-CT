...
class _UnitActionValidator(object):
    ...
    def canDoAction(self, functional):
        ...
        if pInfo.isCreator():
            valid, restriction = self.canCreatorDoAction(pInfo, stats, flags, vInfo)
        else:
            valid, restriction = self.canPlayerDoAction(pInfo, flags, vInfo)
        return (valid, restriction)

    def canCreatorDoAction(self, pInfo, stats, flags, vInfo):
        valid, restriction = self.canPlayerDoAction(pInfo, flags, vInfo)
        if not valid:
            return (valid, restriction)
        ...
        if not valid:
            return (valid, restriction)
        return self._validateSlots(stats, flags)
    ...
    def _validateSlots(self, stats, flags):
