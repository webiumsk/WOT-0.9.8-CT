...
class Minimap(object):
    ...
    def __onFlagSpawnedAtBase(self, flagID, flagPos):
        self.__delFlagMarker(flagID, FLAG_TYPE.NEUTRAL)
        self.__addFlagMarker(flagID, flagPos, FLAG_TYPE.NEUTRAL)
    ...
    def __addFlagCaptureMarkers(self, isCarried = False):
        ...
        for pointIndex, point in enumerate(self.__cfg['flagAbsorptionPoints']):
            ...
            coordinates = Vector3(position.x, 0, position.y)
            isMyTeam = point['team'] == currentTeam
            marker = FLAG_TYPE.ALLY_CAPTURE if isMyTeam else FLAG_TYPE.ENEMY_CAPTURE
            self.__addFlagMarker(pointIndex, coordinates, marker, not isCarried)
            if isMyTeam:
                self.__addFlagMarker(pointIndex, coordinates, FLAG_TYPE.ALLY_CAPTURE_ANIMATION, isCarried)
