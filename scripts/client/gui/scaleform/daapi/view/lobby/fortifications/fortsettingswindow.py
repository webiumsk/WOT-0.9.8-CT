...
class FortSettingsWindow(View, AbstractWindowView, FortSettingsWindowMeta, AppRef, FortViewHelper):
    ...
    def __makePeripheryData(self):
        ...
        return {'peripheryTitle': self.app.utilsManager.textManager.getText(TextType.MAIN_TEXT, i18n.makeString(FORTIFICATIONS.SETTINGSWINDOW_PEREPHERYTITLE)),
         'peripheryName': self.app.utilsManager.textManager.getText(TextType.STANDARD_TEXT if self._isFortFrozen() else TextType.NEUTRAL_TEXT, str(servername)),
    ...
    def __makeDefencePeriodData(self):
        ...
        conditionPostfix = self.app.utilsManager.textManager.getText(TextType.STANDARD_TEXT if self._isFortFrozen() else TextType.NEUTRAL_TEXT, fort.getDefencePeriodStr())
    ...
    def __makeOffDayData(self):
        ...
        conditionPostfix = self.app.utilsManager.textManager.getText(TextType.STANDARD_TEXT if self._isFortFrozen() else TextType.NEUTRAL_TEXT, fort.getOffDayStr())
    ...
    def __makeVacationData(self):
        ...
        if isVacationEnabled:
            ...
            textColor = textColor if not self._isFortFrozen() else TextType.STANDARD_TEXT
    ...
    def __updateNotActivatedView(self):
        ...
        _ms = i18n.makeString
        titleText = _gt(TextType.PROMO_TITLE, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_TITLE))
        description = _gt(TextType.MAIN_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_DESCRIPTION))
        conditionTitle = _gt(TextType.MIDDLE_TITLE, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_CONDITIONTITLE))
        ...
        conditionsText = _gt(TextType.MIDDLE_TITLE, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_CONDITIONS))
        fortConditionsText = _gt(TextType.MAIN_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_CONDITIONS_FORTLEVEL))
        defenceConditionsText = _gt(TextType.MAIN_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_CONDITIONS_DEFENCE))
        attackConditionsText = _gt(TextType.MAIN_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_CONDITIONS_ATTACK))
        firstConditionNotReady = ''
        secondConditionNotReady = ''
        if not self.__checkBaseLevel():
            firstConditionIcon = _gt(TextType.STANDARD_TEXT, '-')
            firstConditionNotReady = _gt(TextType.ERROR_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_ISNOTREADY))
        if not self.__checkPlayerCount():
            secondConditionIcon = _gt(TextType.STANDARD_TEXT, '-')
            secondConditionNotReady = _gt(TextType.ERROR_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_ISNOTREADY))
        firstConditionMsg = _gt(TextType.MAIN_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_BASELEVELCONDITION, level=fort_formatters.getTextLevel(g_fortCache.defenceConditions.minRegionLevel), isNotReady=firstConditionNotReady))
        secondConditionMsg = _gt(TextType.MAIN_TEXT, _ms(FORTIFICATIONS.SETTINGSWINDOW_NOTACTIVATED_PLAYERCOUNTCONDITION, membersCount=BigWorld.wg_getNiceNumberFormat(g_fortCache.defenceConditions.minClanMembers), isNotReady=secondConditionNotReady))
    ...
    def __makeSettingsBlockVO(self, isTopBlock):
        ...
        return {'startLvlSrc': fort_formatters.getIconLevel(minFortLevel),
         ...
         'lvlDashTF': self.app.utilsManager.textManager.getText(TextType.STATS_TEXT, '-'),
