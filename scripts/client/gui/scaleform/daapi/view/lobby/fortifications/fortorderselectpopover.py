        ...
        for orderTypeID in constants.FORT_ORDER_TYPE.CONSUMABLES:
            ...
            if not isBuildingReady:
                ...
                if orderItem.count:
                    ...
                    orderCountText = _getText(TextType.STANDARD_TEXT, _ms(FORTIFICATIONS.ORDERSELECTPOPOVER_ORDERCOUNT, orderNumber=_getText(TextType.ERROR_TEXT, str(orderItem.count))))
            else:
                if isSelectedInThisSlot:
                    description = ''
                else:
