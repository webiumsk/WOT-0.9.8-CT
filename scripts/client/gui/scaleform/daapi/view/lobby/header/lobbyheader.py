...
class LobbyHeader(LobbyHeaderMeta, AppRef, GlobalListener, ClanEmblemsHelper):
    ...
    def __setClanInfo(self, clanInfo):
        ...
        self.as_nameResponseS(g_lobbyContext.getPlayerFullName(name, clanInfo=clanInfo), name, clanAbbrev, g_itemsCache.items.stats.isTeamKiller, g_clanCache.isInClan, hasNew, hasActiveBooster)
    ...
    def __setAccountsAttrs(self, isPremiumAccount, premiumExpiryTime = 0):
        ...
        self.as_setPremiumParamsS(isPremiumAccount, premiumBtnLbl, buyPremiumLabel, canUpdatePremium, disableTTHeader, disableTTBody, hasPersonalDiscount)
    ...
    def __getFightBtnTooltipData(self):
        eventVehicles = g_eventsCache.getEventVehicles()
        eventVehicles.sort(cmp=lambda veh1, veh2: cmp(veh1.intCD, veh2.intCD))
        vehicleNames = ''
        countVehicles = len(eventVehicles)
        for curVehID in xrange(countVehicles):
            curVeh = eventVehicles[curVehID]
            vehicleNames += curVeh.userName
            if curVehID != countVehicles - 1:
                vehicleNames += ','
            else:
                vehicleNames += '.'

        return {'header': i18n.makeString(MENU.HEADERBUTTONS_FIGHTBTN_TOOLTIP_GROUPNOTREADY_HEADER),
         'body': i18n.makeString(MENU.HEADERBUTTONS_FIGHTBTN_TOOLTIP_GROUPNOTREADY_BODY),
         'note': '<br/>' + text_styles.alert(vehicleNames)}
    ...
    def __updatePrebattleControls(self):
        ...
        if not prbDispatcher:
            ...
            if isFightBtnDisabled and isEventVehicle and not state.hasLockedState:
                self.as_setFightBtnTooltipDataS(self.__getFightBtnTooltipData())
            else:
