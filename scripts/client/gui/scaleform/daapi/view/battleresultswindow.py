...
class BattleResultsWindow(View, AbstractWindowView, BattleResultsMeta, AppRef, ClubListener):
    ...
    def __populateAccounting(self, commonData, personalCommonData, personalData, playersData, personalDataOutput):
        ...
        for vehIntCD, sourceData in self.__buildPersonalDataSource(personalData, commonData):
            ...
            if showIntermediateTotal:
                c = int((creditsCell + achievementCredits + orderCredits + boosterCredits + eventCredits - creditsPenalty + creditsCompensation) * aogasFactor)
                cPrem = int((creditsCellPrem + achievementCreditsPrem + orderCreditsPrem + boosterCreditsPrem + eventCredits - int(creditsPenalty * premCreditsFactor) + int(round(creditsCompensation * premCreditsFactor))) * aogasFactor)
            ...
            creditsData.append(self.__getStatsLine())
            creditsWithoutPremTotal = self.__calculateTotalCredits(eventCredits, premCreditsFactor, isPremium, aogasFactor, creditsBase, orderCredits, boosterCredits, creditsToDraw, False)
            ...
            creditsWithPremTotal = self.__calculateTotalCredits(eventCredits, premCreditsFactor, isPremium, aogasFactor, creditsBase, orderCredits, boosterCredits, creditsToDraw, True)
            ...
            if vehIntCD is not None:
                personalData[vehIntCD]['pureCreditsReceived'] = (creditsWithPremTotal if isPremium else creditsWithoutPremTotal) - creditsAutoRepair - creditsAutoEquip - creditsAutoLoad
            ...
            boosterXPPrem = int(round(boosterXP * premXpFactor))
            if boosterXP > 0:
                ...
                xpData.append(self.__getStatsLine(self.__resultLabel('boosters'), boosterXPStr, None, boosterXPPremStr, None))
            ...
            boosterFreeXPPrem = int(round(boosterFreeXP * premCreditsFactor))
            if boosterFreeXP > 0:
                ...
                xpData.append(self.__getStatsLine(self.__resultLabel('boosters'), None, boosterFreeXPStr, None, boosterFreeXPPremStr))
            ...
            if len(xpData) < 7:
                xpData.append(self.__getStatsLine())
            freeXpTotal = self.__makeFreeXpLabel(self.__calculateTotalFreeXp(sourceData, aogasFactor, dailyXpFactor, premXpFactor, igrXpFactor, refSystemFactor, isPremium, freeXpBase, orderFreeXP, boosterFreeXP, eventFreeXP), not isPremium)
            freeXpPremTotal = self.__makeFreeXpLabel(self.__calculateTotalFreeXp(sourceData, aogasFactor, dailyXpFactor, premXpFactor, igrXpFactor, refSystemFactor, isPremium, freeXpBase, orderFreeXP, boosterFreeXP, eventFreeXP, True), isPremium)
            xpData.append(self.__getStatsLine(totalLbl, self.__makeXpLabel(self.__calculateTotalXp(sourceData, aogasFactor, dailyXpFactor, premXpFactor, igrXpFactor, refSystemFactor, isPremium, xpBase, orderXP, boosterXP, eventXP), not isPremium), freeXpTotal, self.__makeXpLabel(self.__calculateTotalXp(sourceData, aogasFactor, dailyXpFactor, premXpFactor, igrXpFactor, refSystemFactor, isPremium, xpBase, orderXP, boosterXP, eventXP, True), isPremium), freeXpPremTotal))
    ...
    def __buildPersonalDataSource(self, personalData, commonData):
        ...
        isFallout = commonData.get('guiType', 0) == ARENA_GUI_TYPE.EVENT_BATTLES
        for vehIntCD, pData in personalData.iteritems():
            data = {}
    ...
    def __buildEfficiencyDataSource(self, pData, pCommonData, playersData, commonData):
        ...
        totalVehsData = {}
        for vehIntCD, data in pData.iteritems():
            enemiesGroup = []
    ...
    def __populateEfficiency(self, pData, pCommonData, playersData, commonData, personalDataOutput):
        ...
        for techniquesGroup, enemiesGroup, basesGroup in self.__buildEfficiencyDataSource(pData, pCommonData, playersData, commonData):
            ...
            for (vId, vIntCD), iInfo in enemiesGroup:
                ...
                result['deathReason'] = iInfo.get('deathReason', 0)
    ...
    def __populateArenaData(self, commonData, pData, commonDataOutput):
        ...
        commonDataOutput['arenaIcon'] = ARENA_SCREEN_FILE.format(arenaType.geometryName)
    ...
    def __calculateTotalCredits(self, eventCredits, premCreditsFactor, isPremium, aogasFactor, baseCredits, baseOrderCredits, baseBoosterCredits, creditsToDraw, usePremFactor = False):
        credits = int(baseCredits - creditsToDraw)
        if usePremFactor:
            credits = int((int(int(baseCredits * premCreditsFactor) - round(creditsToDraw * premCreditsFactor)) + round(baseOrderCredits * premCreditsFactor) + round(baseBoosterCredits * premCreditsFactor) + eventCredits) * aogasFactor)
        return credits

    def __calculateTotalFreeXp(self, pData, aogasFactor, dailyXpFactor, premXpFactor, igrXpFactor, refSystemFactor, isPremium, baseFreeXp, baseOrderFreeXp, baseBoosterFreeXP, eventFreeXP, usePremFactor = False):
        if not baseFreeXp:
            return 0
        ...
        if isPremium != usePremFactor:
            ...
            freeXP = int((resultXp + int(round(baseOrderFreeXp * premXpFactor)) + int(round(baseBoosterFreeXP * premXpFactor)) + eventFreeXP) * aogasFactor)
        return freeXP

    def __calculateTotalXp(self, pData, aogasFactor, dailyXpFactor, premXpFactor, igrXpFactor, refSystemFactor, isPremium, baseXp, baseOrderXp, baseBoosterXP, eventXP, usePremFactor = False):
        xp = pData['xp']
        if isPremium != usePremFactor:
            ...
            if abs(refSystemFactor - 1.0) > 0.001:
                resultXp += int(subtotalXp * refSystemFactor)
            xp = int((resultXp + int(round(baseOrderXp * premFactor)) + int(round(baseBoosterXP * premFactor)) + eventXP + premiumVehicleXP) * aogasFactor)
        return xp
    ...
    def __parseQuestsProgress(cls, battleResults):
        questsProgress = {}
        for data in battleResults.get('personal', {}).itervalues():
    ...
    def __getCommonData(self, callback):
        ...
        if results:
            ...
            statValues = []
            totalStatValues = defaultdict(int)
            for data in personalData.itervalues():
                assisted = data.get('damageAssistedTrack', 0) + data.get('damageAssistedRadio', 0)
                damageAssisted.append(assisted)
                totalDamageAssisted += assisted
                statValues.append(self.__populateStatValues(data, isFallout, True))
